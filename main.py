...
# TRAIN_PATH = r"D:\data\bendV3-800-4\bendV3-800-4\Train\NG"
# TEST_PATH = r"D:\data\bendV3-800-4\bendV3-800-4\Val\NG"

TRAIN_PATH = r"D:\data\bendV3-800-2_adval\Train"
TEST_PATH = r"D:\data\bendV3-800-2_adval\Val"
...
class ProteinDataset(Dataset):
    def __init__(self, size, augment=None):
        ...
        self.train_df = os.listdir(TRAIN_PATH)
        self.test_df = os.listdir(TEST_PATH)

        self.train_df_len = len(self.train_df)
        self.test_df_len = len(self.test_df)

        self.total_length = len(self.train_df) + len(self.test_df)
        print("Train dataset: {}, test dataset {}, total {}".format(self.train_df_len, self.test_df_len,
    ...
    def __getitem__(self, index):
        if index > len(self.train_df) - 1:
            ...
            # ], axis=2)
            img = self._load_image(os.path.join(TEST_PATH,image_id), self.size)
            label = 0
        else:
            ...
            # ], axis=2)
            img = self._load_image(os.path.join(TRAIN_PATH,image_id), self.size)
            label = 1
    ...
    def run(self):
        ...
        while epoch < max_epochs:
            ...
            for inputs, labels in self.train_loader:
                ...
                try:
                    with torch.no_grad():
                        train_acc, train_roc = [i(labels.cpu(), preds.cpu()).item() for i in metrics]
                except ValueError:
                    pass
            ...
            print(
                "{:5f} {:4.1f} {:5.1f} | {:0.3f}* {:0.3f}  {:0.3f}  | {:0.3f}  {:0.3f}  {:0.3f}  | {:6.2f}".format(
                    lr, it / 1000, epoch, valid_loss, valid_roc, valid_acc, train_loss, train_roc, train_acc,
                        timer() - start
                ))
